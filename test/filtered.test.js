const filtered = require('../source/filtered');
...
describe('Function to check filetype', () => {
  it('Checking for txt file type', () => {
    expect(filtered.checkFileType(fileName, filterFileType)).toBeTruthy();
  });

  it('Return false for no file type', () => {
    expect(filtered.checkFileType(fileName)).toBeFalsy();
  });
});
